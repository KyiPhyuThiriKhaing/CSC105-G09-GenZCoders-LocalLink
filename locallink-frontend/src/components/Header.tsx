...
function Header() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 w-full transition-all duration-300 ${scrolled ? "bg-white/85 backdrop-blur-xl shadow-[0_4px_30px_rgba(0,0,0,0.04)] border-b border-white/20" : "bg-white border-b border-transparent"}`}
      >
          ...
          <nav className="flex items-center gap-3 sm:gap-8">
            ...
            >
              <svg
                ...
                className={`transition-transform duration-300 ${isMobileMenuOpen ? "rotate-90 hidden" : "block"}`}
              >
              ...
              <svg
                ...
                strokeLinecap="round"
                strokeLinejoin="round"
                className={`transition-transform duration-300 ${isMobileMenuOpen ? "block" : "hidden"}`}
              >
      ...
      {isLogoutModalOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/40 px-4 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-sm scale-100 rounded-4xl bg-white p-6 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="mt-8 flex gap-3">
              <button
                ...
                className="w-full rounded-2xl border border-slate-200 p-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50"
              >
                Cancel
              </button>
              <button
                onClick={handleLogoutConfirm}
                className="w-full rounded-2xl bg-red-600 p-3 text-sm font-bold text-white transition hover:bg-red-700 shadow-lg shadow-red-600/20"
              >
                Sign Out
              </button>
            </div>
