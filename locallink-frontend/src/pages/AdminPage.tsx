...
export default function AdminPage() {
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-(--color-ink-strong) px-4 py-8">
      <section className="w-full max-w-md rounded-2xl border border-(--color-ink-border-soft) bg-white/6 p-8 shadow-[0_12px_32px_rgba(31,18,51,0.3)]">
        <h1 className="text-center text-3xl font-bold text-(--color-brand-soft)">
          Admin Login
        </h1>
        <p className="mt-2 text-center text-sm text-(--color-brand-accent)">
          Restricted area
        </p>
        ...
        >
          <div>
            <label
              className="mb-2 block text-sm font-medium text-(--color-brand-soft)"
              htmlFor="admin-email"
            >
              Email
            </label>
